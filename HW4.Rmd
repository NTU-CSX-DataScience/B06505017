...
library(rvest)
library(tmcn)
for (index in c(5340:5355)) {
  url = paste0("https://www.ptt.cc/bbs/NBA/index", index, ".html")
  html = read_html(url)
  ...
  getContent <- function(x, id) {
    ...
    filename = paste0(index,'-',id,".txt")
    write.table(text, filename)
  }
...
g <- unlist(ass)
g <- cbind(names(g), g) %>% as.data.frame()
...
plot(1:300, wss, type = "b", xlab = "Number of Clusters",
...
num_clusters
```
利用上面所求得的最佳群集數目，將文章作K-Means Clustering
```{r}
kmeansOut = kmeans(mydata, num_clusters, nstart = 1, algorithm = c("Lloyd"), iter.max = 30)
